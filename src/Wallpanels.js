 ...
 await axios.post( 'https://theaiserver.com/api/v1/infer', body, config).then(res=>{
  ...
  const  resizeImage = async (val, designstyle)=>{
    ...
    // }
     ...
     if(img.width === 1181){
        maxWidth = wallimagewidth/0.7
      maxHeight = wallimageheight/0.7
     }

     if(img.width === 709){
        maxWidth = wallimagewidth/0.9
      maxHeight = wallimageheight/0.9
     }
     
     if(img.width === 576){
        maxWidth = wallimagewidth/0.5
      maxHeight = wallimageheight/0.5
     }

     if(img.width === 14180){
        maxWidth = wallimagewidth
      maxHeight = wallimageheight/8
     }
     if(img.width === 8640){
        maxWidth = wallimagewidth
              ...
              filteredarray && filteredarray.length > 0 ? 
                filteredarray && filteredarray.map((item,i)=>(
                  ...
                  <div class="tooltip" id={`tooltip_${i}`}>
                    <div className='tooltipinside'>
                      ...
                      <p>Size: {item.length} * {item.wide} mm</p>
                      <div className='productpagelinkgrid'>
                      <p>Thickness: {item.Rollsize} mm</p>
                      </div>
                      </div>
                  </div>
                  </label>
               ...
               <div className='wallpaperdetails'>
                 ...
                 <div className='wallpaperdetailsinside'>
                   ...
                   <div className='dimensions'>
                   <p className='productid'>size: {item.length}*{item.wide} mm </p>
                   <p>Thickness: {item.Rollsize} mm</p>
                   </div>
